...
> 历史 receiver 设计（2026-08-09）。后续的 Accelerator 管理、完整 binary 兼容、stackwalk、在线查看和 AI 分析已在 `2026-08-10-accelerator-management-analysis-design.md` 中实现；本文保留初始接收边界和协议背景，不再作为当前完整功能清单。
...
`/symbols/submit` 接收 Breakpad symbol 文本并保存到受控符号目录；`/binary/submit` 接收官方 `code_file` 模块二进制，并按独立大小上限、内容寻址和报告模块引用进行保存。三个端点都要求共享 token、loopback 来源和本项目 managed instance；这不是任意文件上传接口。
...
后续实现已增加“崩溃报告”页面、异步本地 `minidump_stackwalk`、模块/Crash Signature 展示和可选 AI 分析；旧版 API 仍是底层接收兼容边界。

## 5. 安全与故障处理

- 公开接收端只允许 `POST`，强制 token、请求体上限和 multipart 文件上限。
- Compose 中 Panel 使用 host network 并直接监听 `L4D2_PANEL_HTTP_PORT`，不使用 Docker `ports` 映射；游戏服务同样使用 host network，使本机 Accelerator 到 Panel 的来源地址保持 loopback。其他来源即使持有 token 也被拒绝。
...
- 未安装 `minidump_stackwalk` 不阻止接收；当前实现会把它记录为报告级 stackwalk 失败，上传仍然成功。
...
新增 `internal/crashreports` 文件存储与协议处理、Panel 公开接收路由、认证查询 API、配置项和周期清理；后续切片又加入 `internal/accelerator`、本地分析 worker、AI 脱敏和前端页面。现有实例生命周期、Overlay、游戏日志 API、认证会话和 Docker 游戏挂载保持不变；仅 Panel 的网络模式改为 host network 以满足 loopback 来源边界。
