...
前端 App 在认证成功后统一加载并持有该设置，配置尚未完成前禁用 Terminal，避免较大历史在默认值下被提前丢弃；读取失败时回落到 8192 并显示错误提示。SettingsPage 只使用 App 传入的状态和保存回调，不重复读取该接口；保存请求由 App 使正在进行的旧读取失效，保存成功后 Terminal 通过 prop 变化立即裁剪当前文本，不重建 WebSocket。
