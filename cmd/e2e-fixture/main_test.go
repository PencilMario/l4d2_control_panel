...
func TestFixtureGameLogCleanerRemovesExpiredLogs(t *testing.T) {
	root := t.TempDir()
	...
	cleaner := gamelogs.NewCleaner(db, gamelogs.NewManager(root, gamelogs.Options{}))
	if err := cleaner.Cleanup(context.Background(), 14); err != nil {
		t.Fatalf("cleanup error=%v", err)
	}
	aged := filepath.Join(root, "instances", id, "logs", "sourcemod", "errors", "aged-error.log")
	recent := filepath.Join(root, "instances", id, "logs", "sourcemod", "errors", "current-error.log")
	if _, err := os.Stat(aged); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("aged log still exists: %v", err)
	}
