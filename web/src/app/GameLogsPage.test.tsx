...
it('renders nested directories collapsed then expands',async()=>{const api=vi.fn().mockResolvedValue({entries:[{path:'game/logs',kind:'directory'},{path:'game/logs/a',kind:'directory'},{path:'game/logs/a/x.log',kind:'file'}]});render(<GameLogsPage instanceID="i" api={api}/>);await waitFor(()=>screen.getByLabelText('Toggle game/logs'));expect(screen.queryByText('x.log')).toBeNull();screen.getByLabelText('Toggle game/logs').click();screen.getByLabelText('Toggle game/logs/a').click();await waitFor(()=>expect(screen.getByText('game/logs/a/x.log')).toBeTruthy())});
