...
export function VPKUploadQueue({ tasks, onRetry, onCancel }: { tasks: VPKUploadTask[]; onRetry: (task: VPKUploadTask) => void; onCancel: (task: VPKUploadTask) => void }) {
  ...
  return <section className="vpk-upload-queue" aria-label="VPK 上传队列">
    <div className="section-head"><div><h3>上传队列</h3><small>{formatBytes(uploadedBytes)} / {formatBytes(totalBytes)}{totalSpeed > 0 ? ` · ${formatSpeed(totalSpeed)}` : ""}</small></div><small>刷新页面后会自动恢复</small></div>
    {tasks.map((task) => <UploadTask key={task.id} task={task} onRetry={onRetry} onCancel={onCancel} />)}
  </section>;
}
...
function UploadTask({ task, onRetry, onCancel }: { task: VPKUploadTask; onRetry: (task: VPKUploadTask) => void; onCancel: (task: VPKUploadTask) => void }) {
  ...
  return <div className="vpk-upload-task">
    <div className="vpk-upload-task-name"><b>{task.name}</b><small>{task.mode === "clean" ? "上传前清理" : "直接上传"} · {labels[task.status]}</small>{task.phase ? <small>{task.phase} · 已用时 {formatDuration(elapsed)}</small> : null}{task.mode === "clean" && task.removed != null ? <small>移除 {task.removed} 项 · {formatBytes(task.sourceSize)} → {formatBytes(task.size)} · 节省 {savingPercent(task)}%</small> : null}{task.error ? <small className="error-text">{task.error}</small> : null}</div>
    <div className="vpk-upload-progress"><progress max={progressMax} value={progressValue} /><small>{formatBytes(progressValue)} / {formatBytes(progressMax)} · {Math.round((progressValue / progressMax) * 100)}%</small>{task.status === "uploading" ? <small>实时 {formatSpeed(task.speed || 0)} · 平均 {formatSpeed(task.averageSpeed || 0)}{task.etaSeconds != null ? ` · 剩余 ${formatDuration(Math.ceil(task.etaSeconds))}` : ""}</small> : null}</div>
    <div className="inline-actions">{task.status === "failed" ? <button onClick={() => onRetry(task)}>重试</button> : null}<button onClick={() => onCancel(task)}>移除</button></div>
  </div>;
}
